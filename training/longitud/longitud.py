...
# Add interaction term for latitude and longitude (but exclude longitude from features later)
...
# Use top features, excluding longitude (since it's the target)
top_features = ['nst', 'magType_mww', 'magType_mb', 'year', 'latitude',
...
model = Sequential([
    ...
    Dense(1)  # Output layer for longitude
])
...
# Calculate precision percentage (within ±0.5 longitude units)
tolerance = 0.5
...
def plot_confusion_matrix():
    # Adjust bins for longitude range (-75 to -53)
    bins = np.arange(-75, -48, 3)  # -75, -72, ..., -51, -48
